<template>
  <div>
    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
            ...
            <table>
              <tbody>
                ...
                <tr>
                  <th>可提现金额</th>
                  <td>
                    <span style="color: rgb(245, 108, 108)">{{ tablejifen ? `${tablejifen}元` : '0元' }}</span>
                    <!--                    <p>购物返利: </p>-->
                    <!--                    <p>任务金额:-->
                    <!--                      <span style = "color: rgb(245, 108, 108)">{{ renwuMoney ? `${renwuMoney}元` : '0元' }}</span>&nbsp;-->

                    <!--                    </p>-->
                    <span style="color: #4f9dec;cursor:pointer;"
                      @click="withdraw(tableData.id,tableData.jifen)">提现提醒</span>
                    <span style="color: #4f9dec;cursor:pointer;" @click="addMoney">添加金额</span>
                  </td>
                  <th>累计提现金额</th>
                ...
                <tr>
                  <th>可抵扣金额</th>
                  <td style="color: rgb(245, 108, 108)">{{ (userDeductQuota) / 100 ? (userDeductQuota) / 100 : '0' }} 元
                  </td>
                  <!-- <th>不可提现金额</th>
                ...
                <tr>
                  ...
                  <th>图像</th>
                  <td class="border-rt"><img
                      :src="tableData.image_url ? tableData.image_url : 'https://sqx.gomyorder.cn/logo.png'" width="80"
                      height="80" /></td>
                </tr>
                <tr>
                  <th>微信名称</th>
                ...
                <tr>
                  ...
                  <td class="border-rt">{{ tableData.relationId ? tableData.relationId : '未绑定' }} <span
                      style="color: #4f9dec;cursor:pointer;" @click="accredit(tableData.id)">
                      {{ tableData.isInvitation === 0 ? '立即授权' : '取消授权' }}</span> </td>
                </tr>
                <tr>
                  <th>备注</th>
                ...
                <tr>
                  ...
                  <td class=" border-bt">{{ totalnums }}人</td>
      ...
      <el-tab-pane label="订单详情" name="order">
        ...
        <div style="text-align: center;margin-top: 10px;">
          <el-pagination @size-change="handleSizeChange5" @current-change="handleCurrentChange5"
            ...
            layout="total,sizes, prev, pager, next,jumper" :total="orderDatamin.totalElements">
          </el-pagination>
        </div>
      </el-tab-pane>
      ...
      <el-tab-pane label="非直属成员" name="second2">
        <el-table v-loading="tableDataLoading" :data="tableDatamin">
          <el-table-column prop="id" label="编号" align="center" width="80">
          </el-table-column>
          <el-table-column fixed prop="nickName" label="昵称" width="150">
        ...
        <div style="text-align: center;margin-top: 10px;">
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
            :page-sizes="[10, 20, 30, 40]" :page-size="size" :current-page="page"
            layout="total,sizes, prev, pager, next" :total="totalnum">
          </el-pagination>
        </div>
      </el-tab-pane>
      <el-tab-pane label="自营订单" name="self">
        ...
        <div style="text-align: center;margin-top: 10px;">
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
            ...
            layout="total,sizes, prev, pager, next,jumper" :total="selfData.totalElements">
          </el-pagination>
        </div>
      </el-tab-pane>
      ...
      <el-tab-pane label="提现记录" name="sixth">
        <el-table v-loading="tableDataLoading" :data="withdrawData">
          ...
          <el-table-column prop="state" label="状态">
            <template slot-scope="scope">
              <span style="color: #999;" v-if="scope.row.state==-1">已退款</span>
              <span style="color: #009926;" v-if="scope.row.state==0">待转账</span>
              <span style="color: #f56c6c;" v-if="scope.row.state==1">已转账</span>
            </template>
          </el-table-column>
          <el-table-column prop="createAt" label="创建时间">
          </el-table-column>
      ...
      <el-tab-pane label="自营分销" name="fenxiao">
      ...
      <el-tab-pane label="钱包明细" name="eighth">
        ...
        <div style="text-align: center;margin-top: 10px;">
          <el-pagination @size-change="handleSizeChange2" @current-change="handleCurrentChange2"
            :page-sizes="[10, 20, 30, 40]" :page-size="size" :current-page="page"
            layout="total,sizes, prev, pager, next,jumper" :total="walletNum">
          </el-pagination>
        </div>
      </el-tab-pane>
      ...
      <el-tab-pane label="派单记录" name="paidan">
        <div style="display: inline-block;">
          <span>开始时间：</span>
          <el-date-picker style="width: 160px;margin-left: 10px;" v-model="startTime" align="right" type="datetime"
            format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择开始时间">
      ...
      <el-tab-pane label="消息推送" name="tenth">
        <div style="float: right;margin-right:2%;">
          <el-button style="margin: 10px 0;" size="mini" type="primary" icon="document" @click="magNotice">消息推送
          </el-button>
        </div>
        <el-table v-loading="tableDataLoading" :data="userData.content">
      ...
      <el-tab-pane label="卡券明细" name="coupon">
        <el-table v-loading="tableDataLoading" :data="couponData">
          <el-table-column prop="id" label="编号" width="80">
          </el-table-column>
    ...
    <!-- 可提现添加金额 -->
    <el-dialog title="添加金额" :visible.sync="dialogFormVisible2" center>
      <div style="margin-bottom: 10px;">
        <span style="width: 200px;display: inline-block;text-align: right;">金额：</span>
        <el-input style="width:50%;" v-model="Money" type="number" placeholder="请输入金额"></el-input>
      </div>
      <div slot="footer" class="dialog-footer">
    ...
    methods: {
      ...
      dataSelect() {
        ...
        this.$http({
          ...
          if (data.status === 0) {
            ...
            this.tablejifen = returnData.jifen
            this.irrevoSelect()
          }
        })
      },
        ...
        this.$http({
          ...
          this.$http({
            url: this.$http.adornUrl2(
              `/statistics/TeamOrderTotalByRelation?relation=${this.relationId}&tk_status=${tkstatus}&time=${this.info.stockDate}`
              ),
            method: 'get',
            params: this.$http.adornParams({})
          }).then(({
      ...
      teamlastClose(tkstatus) {
        ...
        this.$http({
          ...
          this.$http({
            url: this.$http.adornUrl2(
              `/statistics/TeamOrderTotalByRelation?relation=${this.relationId}&tk_status=${tkstatus}&time=${lastDate}`
              ),
            method: 'get',
            params: this.$http.adornParams({})
          }).then(({
      ...
      irrevoSelect() {
        let userId = this.$route.query.userId
        this.$http({
          url: this.$http.adornUrl2(`/userMoney/selectUserMoney?userId=${userId}`),
          method: 'get',
          data: this.$http.adornData({})
        }).then(({
          data
          ...
          if (money == null) {
            ...
            this.renwuMoney = money
          }
          this.tablejifen = (this.renwuMoney * 1 + this.tablejifen * 1).toFixed(2)
        })
      },
      // 获取直属成员、非直属成员
      teamSelect() {
        ...
        this.$http({
          ...
          if (data.status == 0) {
            ...
            this.$http({
              url: this.$http.adornUrl2(
                `/user/getInvitations/${this.invitation}/${this.page - 1}/${this.size}/${this.type}`),
              method: 'get',
              params: this.$http.adornParams({})
            }).then(({
              data
              ...
              if (this.type == 1) {
                this.totalnums = data.data.totalElements
              }
      ...
      walletSelect() {
        ...
        this.$http({
          ...
          this.walletNum = returnData.totalElements
        })
      },
      // 订单详情
      orderSelect() {
        ...
        this.$http({
          url: this.$http.adornUrl2(
            `/order/orderListByUserId?userId=${userId}&page=${this.page - 1}&size=${this.size}`),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({
          data
          ...
          console.log(typeof returnData.totalElements)
        })
      },
      // 卡券明细
      couponSelect() {
      ...
      StairNoticeTo2() {
        ...
        this.$http({
          url: this.$http.adornUrl2(`/user/addHelpTaskUserMoney?userId=${userId}&amount=${this.Money}`),
          method: 'post',
          data: this.$http.adornData({})
        }).then(({
          data
      ...
      rechargeSelect() {
        ...
        this.$http({
          url: this.$http.adornUrl2(
            `/cash/selectPayDetailsByUserId?page=${this.page - 1}&size=${this.size}&userId=${userId}&startTime=${this.startTime}&endTime=${this.endTime}`
            ),
          method: 'post',
          data: this.$http.adornData({})
        }).then(({
